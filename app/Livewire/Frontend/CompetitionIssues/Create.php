...
class Create extends Component
{
    ...
    public function submitForm()
    {
        ...
        ClubMessageNotefication::create([
            'user_id' => $this->user->id,
            'title' => 'شكوى جديدة من فريق : ' . team()->name,
            'link' => route('dashboard.competition-issues.show', [
                'competition' => $this->competition,
                'competitionIssue' => $issue,
            ]),
            'body' => 'لقد قام فريق ' . team()->name . ' بفتح شكوى جديدة',
        ]);

        session()->flash('success_message', 'تم ارسال الشكوى بنجاح');
    ...
    {
        return view('livewire.frontend.competition-issues.create');
    }
}
