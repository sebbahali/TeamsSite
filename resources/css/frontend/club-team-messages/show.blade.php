<x-frontend.layouts.app>
    ...
    <div class="container">
        <div class="row">
           <div class="card-header bg-primary">
            ...
            <a class="btn btn-success btn-sm mb-2" href="{{ route('frontend.club-team-messages.index') }}" title="رجوع">
                ...
                <div class="card mb-5">
                    <div class="card-body">
                        ...
                        <hr>
                        <div class="row">
                            <div style=font-size:15px class="col-8"><span>الفاضل : </span> {{ $message->target->value }}</div>
  ...
  outline-offset: 0.5em;">{!! $message->body !!}</div>
                        ...
                        @endif
                         <hr>
                        <div class="col-12 mt-5">المرسل : {{ $message->sender()->name }} </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        @foreach ($message->replys as $reply)
                            <x-card.message-reply
                                dir="{{ ($reply->sender->value == \App\Enums\ClubTeamMessageSender::Team->value) ? 'rtl' : 'ltr' }}"
                                image="{{$reply->sender()->logo}}"
                                title="{{ ($reply->sender->value == \App\Enums\ClubTeamMessageSender::Team->value) ? 'انت' : $reply->sender()->name }}"
                                body="{{ $reply->body }}"
                                time="{{ $reply->created_at->diffForHumans() }}"
                            />
                        @endforeach

                        <livewire:frontend.club-team-messages.add-reply :message="$message" />
                    </div>
                </div>
            </div>
        </div>
