<x-dashboard.layouts.app>
    <x-slot name="title">اضافة مباراة جديدة</x-slot>
    <x-slot name="active_url">matches</x-slot>
    <div class="container">
        <div class="row">
            <div class="col-12">
                <x-card.main title="اضافة مباراة جديدة">
                    <form class="my-3" action="{{ route('dashboard.matches.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <x-form.group label="اسم المسابقة" for="competition_name" :error="$errors->first('competition_name')">
                            <x-form.input type="text" id="competition_name" name="competition_name" value="{{ old('competition_name') }}" placeholder="اسم المسابقة" />
                        </x-form.group>

                        <x-form.group label="مكان المباراة" for="match_place" :error="$errors->first('match_place')">
                            <x-form.input type="text" id="match_place" name="match_place" value="{{ old('match_place') }}" placeholder="مكان المباراة" />
                        </x-form.group>
                        ...
                        <x-form.group label="الفريق الخصم" for="opponent_team" :error="$errors->first('opponent_team')">
                            <x-form.input type="text" id="opponent_team" name="opponent_team" value="{{ old('opponent_team') }}" placeholder="الفريق الخصم" />
                        </x-form.group>
                        ...
                        <x-form.group label="التوقيت" for="match_time" :error="$errors->first('match_time')">
                            <x-form.input type="time" id="match_time" name="match_time" value="{{ old('match_time') }}" placeholder="التوقيت" />
                        </x-form.group>

                        <x-form.group label="التاريخ" for="match_date" :error="$errors->first('match_date')">
                            <x-form.input type="date" id="match_date" name="match_date" value="{{ old('match_date') }}" placeholder="التاريخ" />
                        </x-form.group>
                        ...
                        <button class="btn btn-primary" type="submit"><i class="mdi mdi-plus"></i> اضافة المباراة</button>
                    </form>
                </x-card.main>
            </div>
        </div>
