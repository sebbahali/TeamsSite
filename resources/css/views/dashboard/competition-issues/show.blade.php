        ...
        <div class="row">
            <div class="col-12">
                <div class="card mb-5">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-7">وقت الاحتجاج : {{ $competitionIssue->created_at->toDateTimeString() }}</div>

                            @if ($competitionIssue->status != \App\Enums\CompetitionIssueStatus::Close)
                                <div class="col-md-5">
                                    <a href="{{ route('dashboard.competition-issues.close', ['competition' => $competitionIssue->competition->id, 'competitionIssue' => $competitionIssue->id]) }}" class="btn btn-primary">
                                        {{ ($competitionIssue->type == "issue") ? "اغلاق الشكوى" : "اغلاق الرسالة" }}
                                    </a>
                                </div>
                            @endif
                    ...
                    <div class="card-body">
                        @foreach ($competitionIssue->replys as $reply)
                            <x-card.message-reply
                                dir="{{ ($reply->sender->value == \App\Enums\CompetitionIssueSender::Club->value) ? 'rtl' : 'ltr' }}"
                                image="{{$reply->sender()->logo}}"
                                title="{{ ($reply->sender->value == \App\Enums\CompetitionIssueSender::Club->value) ? 'انت' : $reply->sender()->name }}"
                                body="{{ $reply->body }}"
                        ...
                        @if ($competitionIssue->status == \App\Enums\CompetitionIssueStatus::Open)
                            <livewire:dashboard.competition-issues.add-reply :issue="$competitionIssue" />
                        @else
