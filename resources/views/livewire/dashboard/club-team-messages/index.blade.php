            ...
            <x-card.main title="رسائل الفرق">
                <div class="form-inline d-flex align-items-center border-bottom mb-2">
                    ...
                    <div class="form-check mx-sm-2 align-self-end">
                        <a href="{{ route('dashboard.club-team-messages.create') }}" class="btn btn-primary btn-sm" title="ارسال رسالة جديدة">
                            <i class="mdi mdi-message-plus"></i>
                        </a>
                    </div>

                    <div class="form-check mx-sm-2 align-self-end">
                        <a href="{{ route('dashboard.club-team-messages.group-message-create') }}" class="btn btn-primary btn-sm" title="ارسال رسالة جديدة">
                            <i class="mdi mdi-message-plus"></i> رسالة جماعية
                        </a>
                    </div>
                </div>
                ...
                <div class="preview-list">
                    @forelse ($messages as $message)
                        <a href="{{ route('dashboard.club-team-messages.show', $message->id) }}"
                            class="dropdown-item preview-item">
                            <div class="preview-thumbnail">
                                <img src="{{$message->sender()->logo}}"
                            ...
                            <div
                                ...
                                <i style=font-size:12px class="mdi mdi-account-circle"> من: {{ $message->sender()->name }}: {{ $message->team()->name }}</i>
                                    @if ($message->status)
                                        <i class="mdi mdi-checkbox-multiple-marked-circle"></i> مقروء
                                    @else
                                        <i class="mdi mdi-checkbox-multiple-blank-circle"></i> غير مقروء
                                    @endif
                                    <i style=font-size:12px class="mdi mdi-clock"></i>
